...
public class CombatAssistant{
	...
	public int[][] donatorNpcs = {{180,1},{182,1},{183,1}};
	
	public boolean donatorNpcs1(int i) {
		for (int j = 0; j < donatorNpcs.length; j++) {
			if (donatorNpcs[j][0] == Server.npcHandler.npcs[i].npcType) {
				if (donatorNpcs[j][1] > c.memberStatus) {
					c.sendMessage("Only donators can harm this npc.");
					return false;
				}
			}
		}
		...
		if (Server.npcHandler.npcs[i] != null) {
			...
			if(c.projectileStage == 0 && !c.usingMagic && !c.castingMagic) { // melee hit damage
				...
				if(c.doubleHit && !c.usingClaws) {
					applyNpcMeleeDamage(i, 2, Misc.random(calculateMeleeMaxHit() + 10));
				}
				if(c.doubleHit && c.usingClaws) {
	...
	public void playerDelayedHit(int i) {
		if (Server.playerHandler.players[i] != null) {
			...
			if(c.projectileStage == 0 && !c.usingMagic && !c.castingMagic) { // melee hit damage								
					...
					if(c.doubleHit && !c.usingClaws) {
						applyPlayerMeleeDamage(i, 2, Misc.random(calculateMeleeMaxHit() + 3));
					}	
					if(c.doubleHit && c.usingClaws) {
...
public void appendMultiBarrageNPC(int npcId, boolean splashed) {
	...
	public void activatePrayer(int i) {
		...
		int[] defPray = {0,5,13,24,25,26};
		int[] strPray = {1,6,14,24,25,26};
		int[] atkPray = {2,7,15,24,25,26};
	...
	**/
	...
	public void activateSpecial(int weapon, int i){
		...
		switch(weapon) {
			...
			case 5680:
			case 5698:
			/*if (c.playerWeapon == 1215 || c.playerWeapon == 1231 || c.playerWeapon == 5680 || c.playerWeapon == 5698) {*/
			c.gfx100(252);
	...
	**/
	...
	public boolean checkReqs() {
		...
		if(Server.playerHandler.players[c.playerIndex].isInPVPSafe()) {
		...
		if(c.isInPVPSafe()) {
		c.sendMessage("You cannot attack other players in a safe zone!");
		c.stopMovement();
		c.getCombat().resetPlayerAttack();
			return false;
		}
	...
	**/
	...
	public void getPlayerAnimIndex(String weaponName){
		...
		switch(c.playerEquipment[c.playerWeapon]) {	
			...
			case 15286:
			c.playerStandIndex = 808;
			c.playerWalkIndex = 2693;
			c.playerRunIndex = 2693;
			break;
			case 11694:
			case 11696:
			case 11730:
	...
	**/
	...
	public int getAttackDelay(String s) {
		...
		//sara sword, 2400ms
			...
			return 7;
		else if(s.equals("chaotic maul"))
			return 9;
		else if(s.equals("chaotic longsword"))
			return 6;
		else if(s.equals("chaotic staff"))
			return 5;
	...
	**/
	...
	public int calculateMeleeAttack() {
		...
		if (c.prayerActive[2]) {
			...
			attackLevel += c.getLevelForXP(c.playerXP[c.playerAttack]) * 0.38;
		}
		if (c.fullVoidMelee()) {
					attackLevel += c.getLevelForXP(c.playerXP[c.playerAttack]) * 0.1;
		}
	...
	public int calculateMeleeDefence() {
		...
		if (c.prayerActive[0]) {
			...
			defenceLevel += c.getLevelForXP(c.playerXP[c.playerDefence]) * 0.35;
		}
