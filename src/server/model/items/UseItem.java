...
public class UseItem {
	...
	public static void ItemonObject(Client c, int objectID, int objectX, int objectY, int itemId) {
		...
		if (objectID == 377 && itemId == 989 && objectX <= 3097 && objectY >= 3499) {
			c.getItems().deleteItem(itemId, c.getItems().getItemSlot(itemId), 1);
			int random = Misc.random(2);
			int random2 = Misc.random(10);
				if(random == 2 && random2 == 1) {
						c.getItems().addItem(c.crystalchest(), 1);	
						c.sendMessage("You loot the chest.");						
				} else if (random == 2 && random2 > 1) {
						c.getItems().addItem(c.crystalchestjunk(), 1);
						c.sendMessage("You find some gloves inside the chest.");						
				} else {
					c.getItems().addItem(995, (Misc.random(15000000) + 1));
					c.sendMessage("You find some gold inside the chest!");
			}
		}
	}
