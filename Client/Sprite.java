...
public final class Sprite extends DrawingArea {
	...
	public String location = sign.signlink.findcachedir() + "Data/Sprites/";

	public Sprite(byte abyte0[], Component component) {
		try {
			Image image = Toolkit.getDefaultToolkit().getImage(location+"Login/bg.jpg");
    ...
    public Sprite(String img) {
		...
		if (img.equalsIgnoreCase("tabhover") || img.equalsIgnoreCase("tabclicked")) {
                		myWidth = 32;
                		myHeight = 37;
            		}
		if (img.equalsIgnoreCase("MAPBACK 1")) {
                    		myWidth = 249;
                    		myHeight = 168;
		}
		if (img.equalsIgnoreCase("titlebox")) {
				myWidth = 360;
				myHeight = 200;
		} else if (img.equalsIgnoreCase("titlebutton")) {
				myWidth = 147;
				myHeight = 41;
		}
            		for(int i = 0; i <= 14; i++) {
