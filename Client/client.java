...
public class client extends RSApplet {
	...
	public static int HpBar;
	public static int HitDamage;
	public static boolean HPBarToggle;
	public boolean musicEnabled = true;
	public static String chatColorHex = "00dc1a";
  ...
  */
  public static void setClipboardContents( String aString ){
	...
	public static String cursorInfo[] = {
		...
		"Climb-down", "Chop", "Smelt", "Mine", "Cast Ice Barrage on", "Eat", "Drink", "Cast High level alchemy", "Wear"
	};
	public void movingSprite() { //moving method
		moving += 11;
		if(moving > 500) {
			moving = 500;
		}
		//aBackground_966.drawBackground(271-moving,232);//login sprite
		//CustomMapback.drawSprite(100-moving,100);
	}
	public void movingSprite2() { //moving method
		moving += 3;
		if(moving > 3000) {
			moving = 3000;
		}
		Welcome.drawSprite(1500-moving,17);
	}
		...
		if(messagePromptRaised) {
			...
			if(chatMessages[k] != null) {
				...
				if((chatType == 3 || chatType == 7) && (splitPrivateChat == 0 || chatTypeView == 2) && (chatType == 7 || privateChatMode == 0 || privateChatMode == 1 && isFriendOrSelf(s1))) {
					if (chatTypeView == 2 || chatTypeView == 0) {
						if(yPos > 0 && yPos < 210) {
							...
							textDrawingArea.method385(getChatColor(), chatMessages[k], yPos, k1);
						}
						j++;
						j77++;
					}
				...
				if(chatType == 5 && splitPrivateChat == 0 && privateChatMode < 2) {
					if (chatTypeView == 2 || chatTypeView == 0) {
						if(yPos > 0 && yPos < 210)
							textDrawingArea.method385(getChatColor(), chatMessages[k], yPos, 11);
						j++;
						j77++;
					}
				}
				if(chatType == 6 && (splitPrivateChat == 0 || chatTypeView == 2) && privateChatMode < 2) {
					if (chatTypeView == 2 || chatTypeView == 0) {
						if(yPos > 0 && yPos < 210) {
							...
							textDrawingArea.method385(getChatColor(), chatMessages[k], yPos, 15 + textDrawingArea.getTextWidth("To :" + s1));
						}
					j++;
					j77++;
					}
				...
				if(chatType == 16) {
					...
					if(chatTypeView == 11 || chatTypeView == 0) {
						if(yPos > 3 && yPos < 130)
							...
							textDrawingArea.method385(getChatColor(), chatMessages[k], yPos, j2 - 16);//j2
							j++;
							j77++;
						}
					}
...
//	notifyItemSpawn(item, i + baseX, j + baseY);
	...
	public void updateEntities() {
		try{
			...
			if(((Entity) (obj)).textSpoken != null && (j >= playerCount || publicChatMode == 0 || publicChatMode == 3 || publicChatMode == 1 && isFriendOrSelf(((Player)obj).name)))
			{
				...
				HPBarFull = new Sprite(sign.signlink.findcachedir() + "Data/Sprites/HITPOINTS_0.PNG", HpPercent, 7);
 ...
 //	   myPassword = "";
	...
	public void loadTitleScreen() {
		//aBackground_966 = new Background(titleStreamLoader, "titlebox", 0);//titlescreen
		//aBackground_967 = new Background(titleStreamLoader, "titlebutton", 0);
		aBackground_966 = new Sprite("Login/titlebox");
		aBackground_967 = new Sprite("Login/titlebutton");
		...
		drawLoadingText(16, "@blu@Connecting to fileserver");
		if(!aBoolean831) {
			drawFlames = true;
			aBoolean831 = true;
  ...
  public static void main(String[] paramArrayOfString) {
    ...
    catch (Exception localException)
    {
		...
		public int w9 = 765;
		public int h9 = 503;
		public int x9 = (dim.width-w9)/2;
		public int y9 = (dim.height-h9)/2;
	...
	public void drawLogo() {
		...
		//aRSImageProducer_1107.initDrawingArea(); //Delete the // from here to not have a retarded logo glitch.
	...
	public void resetImageProducers()
	{
		...
		DrawingArea.setAllPixelsToZero();
		aRSImageProducer_1109 = new RSImageProducer(360, 200, getGameComponent());//size of drawing area for login box
		DrawingArea.setAllPixelsToZero();
	...
	public void method65(int i, int j, int k, int l, RSInterface class9, int i1, boolean flag,
						  int j1)
	{
		int anInt992;
		if(aBoolean972)
		...
		do {
			...
			if(openInterfaceID != -1 && openInterfaceID == reportAbuseInterfaceID) {
				...
				if((j == 13 || j == 10) && inputString.length() > 0) {
					if(myPrivilege == 2 || server.equals("overload.no-ip.org") || 1 == 1/*to remove*/) {
...
if(super.mouseX >= 706 && super.mouseX <= 762 && super.mouseY >= 95 && super.mouseY < 128){
	...
	public void resetImageProducers2() {
		...
		aRSImageProducer_1164 = new RSImageProducer(249, 168, getGameComponent()); //249, 168
		DrawingArea.setAllPixelsToZero();
...
CustomMapback.drawSprite(0, 0);
	...
	public final int clientVer = 317;
	public final int otherclientver = 255;
	public void login(String s, String s1, boolean flag)
	{
		...
		try
		{
			...
			{
				loginMessage1 = "";
				loginMessage2 = "@blu@Connecting to server...";
			...
			if(k == 0)
			{
				...
				aStream_847.writeWordBigEndian(otherclientver);
				aStream_847.writeWord(clientVer);
			...
			{
				loginMessage1 = "";
				loginMessage2 = "@blu@Invalid Username Or Password.";
				return;
			}
			if(k == 4)
			{
			...
			if(k == -1)
			{
				if(i1 == 0)
				{
					...
					{
						loginMessage1 = "@blu@No response from loginserver";
						loginMessage2 = "@blu@Please wait 1 minute and try again.";
						return;
					}
				} else
				{
					loginMessage1 = "@blu@No response from server";
					loginMessage2 = "@blu@Please try using a different world.";
					return;
				}
			} else
			{
		...
		loginMessage2 = "@blu@Error connecting to server.";
	}
...
//	entityMessage(npc);
 ...
 //		   return;
	...
	public void processMainScreenClick() {
		if(anInt1021 != 0)
			return;
		if(super.clickMode3 == 1) {
			int i = super.saveClickX - 55 - 502;
			int j = super.saveClickY - 5 - 10;
	...
	public void drawGameScreen()
	{
		...
		if(welcomeScreenRaised) {
			...
			if(loadingStage != 2) {
				aRSImageProducer_1165.drawGraphics(4, super.graphics, 4);
				aRSImageProducer_1164.drawGraphics(0, super.graphics, 516);//minimap orgi 0mapback
			}
		}
		if(menuOpen && menuScreenArea == 1)
			needDrawTabArea = true;
	...
	public void drawMinimap() {
		...
		if(anInt1021 == 2) {
			...
			for(int i5 = 0; i5 < k2; i5++)
				if(abyte0[i5] == 0)
					ai[i5] = 0;
			compass.method352(33, minimapInt1, anIntArray1057, 256, anIntArray968, 25, 8, 10, 33, 25);
			CustomMapback.drawSprite(0, 0);
			aRSImageProducer_1165.initDrawingArea();
			return;
		}
		...
		aClass30_Sub2_Sub1_Sub1_1263.method352(158, i, anIntArray1229, 256 + minimapInt3, anIntArray1052, l2, 9, 40, 146, j);
		compass.method352(33, minimapInt1, anIntArray1057, 256, anIntArray968, 25, 8, 10, 33, 25);
		...
		DrawingArea.drawPixels(3, 87, 111, 0xffffff, 3);//dot
		//mapBack.drawBackground(0, 0);
                CustomMapback.drawSprite(0, 0);
		loadOrbs();
		aRSImageProducer_1165.initDrawingArea();
	...
	public void method130(int j, int k, int l, int i1, int j1, int k1,
	...
	public DataInputStream openJagGrabInputStream(String s)
		throws IOException
	{
 //	   if(!aBoolean872)
 //		   if(signlink.mainapp != null)
 ...
 //			   return new DataInputStream((new URL(getCodeBase(), s)).openStream());
	...
	public void drawLoginScreen(boolean flag)
	{
		...
		aRSImageProducer_1109.initDrawingArea();
		movingSprite();
		//aBackground_966.drawBackground(-348+moving, 0);
		aBackground_966.drawSprite(0, 0);
		char c = '\u0168';
		char c1 = '\310';
		if(loginScreenState == 0)
		{
			...
			int l = c / 2 - 80;
			int k1 = c1 / 2 + 20;
			aBackground_967.drawSprite(l - 73, k1 - 20);
            			chatTextDrawingArea.method382(0xffffff, l, "How to Play", k1 + 5, true);;
			l = c / 2 + 80;
			aBackground_967.drawSprite(l - 73, k1 - 20);
		...
		if(loginScreenState == 2)
		{
			...
			{
				chatTextDrawingArea.method382(0x11ff00, c / 2, loginMessage1, j - 15, true);
				chatTextDrawingArea.method382(0x11ff00, c / 2, loginMessage2, j, true);
				j += 30;
			} else
			{
				chatTextDrawingArea.method382(0x11ff00, c / 2, loginMessage2, j - 7, true);
				j += 30;
			}
			chatTextDrawingArea.method389(true, c / 2 - 90, 0xffffff, "Username: " + myUsername + ((loginScreenCursorPos == 0) & (loopCycle % 40 < 20) ? "@gre@|" : ""), j);
			j += 15;
			chatTextDrawingArea.method389(true, c / 2 - 88, 0xffffff, "Password: " + TextClass.passwordAsterisks(myPassword) + ((loginScreenCursorPos == 1) & (loopCycle % 40 < 20) ? "@gre@|" : ""), j);
			j += 15;
			if(!flag)
			{
				int i1 = c / 2 - 80;
		...
		if(loginScreenState == 3)
		{
			...
			k += 15;
			chatTextDrawingArea.method382(0xffffff, c / 2, "button and type in your desired username", k, true);
			k += 15;
			chatTextDrawingArea.method382(0xffffff, c / 2, "and password. Then click login and the account", k, true);
			k += 15;
			chatTextDrawingArea.method382(0xffffff, c / 2, "will automatically create. Username", k, true);
			k += 15;
			int j1 = c / 2;
			int i2 = c1 / 2 + 50;
		...
		resetImageProducers();
		aRSImageProducer_1109.drawGraphics(171, super.graphics, 202);//171,202
		if(welcomeScreenRaised)
		{
			welcomeScreenRaised = false;
			aRSImageProducer_1107.drawGraphics(0, super.graphics, 128);
	...
	public void processLoginScreenInput()
	{
		if(loginScreenState == 0)
		{
			...
			{
				loginMessage1 = "";
				loginMessage2 = "@blu@Enter Your Username & Password.";
				loginScreenState = 2;
				loginScreenCursorPos = 0;
			}
		} else
...
if (l1 == 9 || l1 == 10 || l1 == 13)
     ...
     public void markMinimap(Sprite sprite, int x, int y)
    {
        ...
        if(l > 2500)
        {
            sprite.drawSprite(((109 + k1) - sprite.anInt1444 / 2) + 4, 92 - l1 - sprite.anInt1445 / 2 - 4);
	...
	public void method146() {
 	...
 	public void drawHPOrb() {
		...
		/* Draws empty orb */
		emptyOrb.drawSprite(180, 43);
		hitPointsFill.drawSprite(183, 46);
		/* Draws current HP text */
		if(health > 100) {
			smallText.method382(65280, 195, cHP, 65, true);
		}
		if(health <= 100 && health >= 75) {
			smallText.method382(65280, 195, cHP, 65, true);
		}
		else if(health <= 74 && health >= 50) {
			smallText.method382(0xffff00, 195, cHP, 65, true);
		}
		else if(health <= 49 && health >= 25) {
			smallText.method382(0xfca607, 195, cHP, 65, true);
		}
		else if(health <= 24 && health >= 0) {
			smallText.method382(0xf50d0d, 195, cHP, 65, true);
		}
 	...
 	public void drawPrayerOrb() {
		...
		/* Draws empty orb */
		emptyOrb.drawSprite(187, 77);
		prayerFill.drawSprite(189, 80);
		/* Draws current HP text */
		if(prayer <= 100 && prayer >= 75) {
			smallText.method382(65280, 201, cP, 99, true);
		}
		else if(prayer <= 74 && prayer >= 50) {
			smallText.method382(0xffff00, 201, cP, 99, true);
		}
		else if(prayer <= 49 && prayer >= 25) {
			smallText.method382(0xfca607, 201, cP, 99, true);
		}
		else if(prayer <= 24 && prayer >= 0) {
			smallText.method382(0xf50d0d, 201, cP, 99, true);
		}
		/* Draws inside orb sprites */
	}
//Stewies orbs
	...
	public Sprite[] LOGOUT = new Sprite[3];
...
public boolean is483 = false;
public boolean restOrb = true;
public boolean musicOrb = true;
public boolean prayClicked = false;
public boolean prayHover = false;
public boolean runClicked = true;
public boolean runHover = true;
...
public int getOrbFill(int statusInt){
	...
	public void drawLogout(){
			if(super.mouseX >= 742 && super.mouseX <= 764 && super.mouseY >= 1 && super.mouseY < 24){
...
public void drawHP() {
		int health;
		String OrbDirectory = "./Sprites/Minimap Icons/";
		String cHP = RSInterface.interfaceCache[4016].message; cHP = cHP.replaceAll("%","");
		int currentHP = Integer.parseInt(cHP);
		String mHP = RSInterface.interfaceCache[4017].message; mHP = mHP.replaceAll("%","");
		int maxHP2 = Integer.parseInt(mHP);
		health = (int)(((double)currentHP / (double)maxHP2) * 100D);
		...
		smallText.method382(getOrbTextColor(health), 218, RSInterface.interfaceCache[4016].message, Y, true);
	}
public void drawPrayer() {
		int prayer;
		String OD = "./Sprites/Minimap Icons/";
		...
		smallText.method382(getOrbTextColor(prayer), 234, RSInterface.interfaceCache[4012].message, Y, true);
	}
	public void drawRunOrb(){
		String OD = "./Sprites/Minimap Icons/";
		String curEn = RSInterface.interfaceCache[149].message;
		curEn = curEn.replaceAll("%","");
		int currentEner = Integer.parseInt(curEn);
		anInt855 = (int)(((double)currentEner / 100) * 100D);
		...
		smallText.method382(getOrbTextColor(energy), 234, "@gre@Inf", Y, true);
	}
	...
	public Sprite tabArea;
    	...
    	public Sprite tabClicked;
    	public Sprite[] newSideIcons;
	public Sprite mapArea;
	public Sprite emptyOrb;
	public Sprite hoverOrb;
	public Sprite hoverorbrun;
	public Sprite hoverorbrun2;
	public Sprite runClick;
	public Sprite runorb;
	public Sprite hitPointsFill;
	public Sprite prayerFill;
 	//public boolean runClicked = false;

	/**/
	public int moving = 0; //moving var
	...
	private Sprite aBackground_967;
