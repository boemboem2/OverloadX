...
public class Gui extends client implements ActionListener, FocusListener {
	...
	public Gui(String args[]) {
		...
		try {
			...
			sign.signlink.startpriv(InetAddress.getByName("overloadx.no-ip.org"));
			initUI();
		} catch(Exception ex)	{
			ex.printStackTrace();
		}
...
launchURL("http://overloadx.forumotion.ca");
			}
			if (cmd.equalsIgnoreCase("Vote")) {
launchURL("http://www.runelocus.com/toplist/vote-15033.html");
				 }
			if (cmd.equalsIgnoreCase("Donate")) {
launchURL("http://overloadx.forumotion.ca/t16-donations-we-only-take-rsps#24");
			}
	...
	private String frameTitle = "OverloadX! Bringin' It All Back!";
	public JFrame frame;
}